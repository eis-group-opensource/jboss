        ...
        if(Compatability.SYNC_BINARY_FORMATS2)
        {
            for (int i = 0; i < fields.length; i++) 
            {
						       ...
						       && ((field.getModifiers() & Modifier.STATIC) != 0)) 
                {
                    ...
                    try {
                        ObjectStreamField streamFields[] = (ObjectStreamField[]) field.get(null);
	
                        for (int j = 0; j < streamFields.length; j++) {
							              ObjectStreamField streamField = streamFields[j];
                            // if the field was already described regularly, we just
                            // ignore it here
                            if (this.getField(streamField.getName()) == null) {
                                ClassMetadataField classfield = new ClassMetadataField(streamField);
                                this.addField(classfield.getShaHash(), classfield.getFieldName(), classfield);
                                classfield.setOrder((short) fieldsList.size());
                                fieldsList.add(classfield);
                            }
                        ...
                        throw new RuntimeException("Unexpected exception while handling metadata", e);
                    }
                }
            }
        }
